...
module.exports = async ({ getNamedAccounts, deployments }) => {
    ...
    if (await getChainId() !== '31337') {
        await hre.run('verify:verify', {
            ...
            constructorArguments: [constants.WETH[chainId], constants.LOP[chainId], constants.FEE_COLLECTOR_OWNER[chainId]],
        });

        await hre.run('verify:verify', {
            address: feeCollectorFactory.address,
            constructorArguments: [feeCollector.address, constants.FEE_COLLECTOR_FACTORY_OWNER[chainId]],
        });
    }
};
