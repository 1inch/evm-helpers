...
interface IMaverickV1Pool {
    event Swap(address indexed sender, address indexed recipient, bool tokenAIn, bool exactOutput, uint256 amountIn, uint256 amountOut, int32 activeTick);
    event AddLiquidity(address indexed sender, uint256 indexed tokenId, BinDelta[] binDeltas);
    event MigrateBinsUpStack(address indexed sender, uint128[] binIds, uint32 maxRecursion);
    ...
    /// @notice return paramters for Add/Remove liquidity
    ...
    /// @param reserveA amount of B token in bin
    ...
    /// @param binIds is an array of the binIds to be migrated
    /// @param maxRecursion is the maximum rerursion depth of the migration. set to
    //zero to recurse until the active bin is found.
    function migrateBinsUpStack(uint128[] calldata binIds, uint32 maxRecursion) external;
    /// @notice swap tokens
