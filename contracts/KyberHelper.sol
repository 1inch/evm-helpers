...
contract KyberHelper {
     ...
     */
   function getTicks(IKyber pool, uint maxTickNum) external view returns (bytes[] memory ticks) { // solhint-disable-line explicit-types
        (,,int24 tick,) = pool.getPoolState();

        int24[] memory initTicks = new int24[](maxTickNum);

        uint counter = 1; // solhint-disable-line explicit-types
        initTicks[0] = tick;
