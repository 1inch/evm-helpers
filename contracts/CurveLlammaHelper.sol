...
contract CurveLlammaHelper {
    function get(address pool) public view returns(bytes memory) {
        ...
        assembly {
            ...
            let success := staticcall(gas(), pool, in, 0x04, add(res, 0x40), 0x20)
            if eq(success, 0) {
                revert(in, 0x04)
            }
            ...
            success := staticcall(gas(), pool, in, 0x24, add(res, 0x60), 0x20)
            if eq(success, 0) {
                revert(in, 0x24)
            }

            mstore(in, shl(224, 0xca72a821)) // min_band()
            success := staticcall(gas(), pool, in, 0x04, add(res, 0x80), 0x20)
            if eq(success, 0) {
                revert(in, 0x04)
            }
            ...
            success := staticcall(gas(), pool, in, 0x04, add(res, 0xA0), 0x20)
            if eq(success, 0) {
                revert(in, 0x04)
            }
            ...
            success := staticcall(gas(), pool, in, 0x04, add(res, 0xC0), 0x20)
            if eq(success, 0) {
                revert(in, 0x04)
            }

            mstore(in, shl(224, 0x77c34594)) // dynamic_fee()
            success := staticcall(gas(), pool, in, 0x04, add(res, 0xE0), 0x20)
            if eq(success, 0) {
                revert(in, 0x04)
            }

            mstore(in, shl(224, 0xfee3f7f9)) // admin_fee()
            success := staticcall(gas(), pool, in, 0x04, add(res, 0x100), 0x20)
            if eq(success, 0) {
                revert(in, 0x04)
            }
            ...
            for { let i := min } or(gt(i, m), lt(i, max)) { i := add(i, 1) } {
                ...
                mstore(add(in, 4), i)
                success := staticcall(gas(), pool, in, 0x24, add(res, add(p, 0x20)), 0x20)
                if eq(success, 0) {
                    revert(in, 0x24)
                }
